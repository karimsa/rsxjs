...
export class Channel<T> implements IChannel<T> {
	...
	private async _take(
		...
		if (this.buffer.length === 0) {
			const nextPutter =
				dir === 'left' ? this.putters.shift() : this.putters.pop()
			if (nextPutter) {
				nextPutter.signal.resolve()
				debug(
					`taker found an empty buffer & a background putter, stealing value`,
				)
				return {
					ok: true,
			...
			if (isDefined(timeout)) {
				const [i, value] = await any([p.promise, delay(timeout)])
		...
		const poppedValue =
			dir === 'left' ? (this.buffer.shift() as T) : (this.buffer.pop() as T)
	...
	writeOnly(): WriteOnlyChannel<T> {
		return (this.writeOnlyChan =
			this.writeOnlyChan || new WriteOnlyChannel(this))
	}
}

export class ReadOnlyChannel<T> {
	constructor(private readonly chan: Channel<T>) {}
